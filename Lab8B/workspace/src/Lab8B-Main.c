...
void __attribute__((weak)) Bills(uint32_t dollars, BILLS *paper)
    {
    paper->twenties = dollars / 20 ;    dollars -= 20 * paper->twenties ;
    paper->tens     = dollars / 10 ;    dollars -= 10 * paper->tens ;
    paper->fives    = dollars /  5 ;    dollars -=  5 * paper->fives ;
    paper->ones     = dollars ;
    }

void __attribute__((weak)) Coins(uint32_t cents, COINS *coins)
    {
    coins->quarters = cents / 25 ;      cents -= 25 * coins->quarters ; 
    coins->dimes    = cents / 10 ;      cents -= 10 * coins->dimes ;
    coins->nickels  = cents /  5 ;      cents -=  5 * coins->nickels ;
    coins->pennies  = cents ;
    }
...
int main()
    ...
    InitializeHardware(HEADER, "Lab 8B: Making Change") ;
